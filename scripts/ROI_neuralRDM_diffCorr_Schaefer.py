...
if __name__ == "__main__":
    ...
    # load the re-ordered trait list
    traitDf = pd.read_csv(os.path.join(root_dir, 'RSA', 'inputs', 'traitRating_reorder.csv'))

    # extract the trait as the reference order
    ref_order = traitDf['positiveTrait'].tolist()
    ...
    for subID in sub_list:
        ...
        sub_neuralRDM_dir = os.path.join(root_dir,'RSA', 'neuralRDM_Schaefer', f'sub-{subID}')
        ...
        preLen = len("/RSA/neuralRDM_Schaefer/sub-01/sub-01_friend_order-1_ROI-")
        ...
        preLen = len("/RSA/neuralRDM_Schaefer/sub-01/sub-01_school_order-1_ROI-")
    ...
    output_dir = os.path.join(root_dir,'RSA', 'outputs', f'neuralRDM_diffCorr_ROI_F{friendOrder}S{schoolOrder}_Schaefer.csv')
