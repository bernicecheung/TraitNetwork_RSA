...
if __name__ == "__main__":
    ...
    for subID in sub_list:
        ...
        for order, run in enumerate(context_runNum, start=1):
            ...
            # load the reordered-beta data
            beta_dir = os.path.join(root_dir, 'RSA', 'reorder_beta',
                                    f'sub-{subID}_task-{context}_run-{run}_*.nii.gz')
            ...
            for ROI_idx, m in enumerate(mask_x):
                ...
                similarityMatrix = 1 - beta.apply_mask(m).distance(metric='correlation')
                ...
                similarityMatrix_dir = os.path.join(root_dir,'RSA', 'neuralRDM_Schaefer', f'sub-{subID}',
    ...
    # write the dataframe
    output_dir = os.path.join(root_dir,'RSA', 'outputs', f'neuralRDM_sameCorr_ROI_{context}_Schaefer.csv')
